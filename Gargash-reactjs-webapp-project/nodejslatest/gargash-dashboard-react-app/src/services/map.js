...
export function settingLayer(overlayRef, allTrackers, webSocketData, ) {
  overlayRef.current.setProps({
    layers: [
      new IconLayer(overlayBuilder(allTrackers, 'icon-layer-vehicle', webSocketData))
    ]
  });
}
    ...
    updateState((prevWebSocketGroup) => {
      if (!prevWebSocketGroup) {
        ...
        const updatedData = prevWebSocketGroup.map((prevData) => {
          const newData = data.find((d) => d.state.source_id === prevData.sourceId);
